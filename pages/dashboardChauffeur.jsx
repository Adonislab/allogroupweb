  ...
  const formatDateTime = (timestamp) => {
    ...
    const deliveryInfos = (item) => {
      alert(`
        ...
        Adresse de départ: ${item.addressRecuperation}
        Destinataire: ${item.numeroALivraison}
        Adresse de destination: ${item.addressLivraison}
        Colis: ${item.title}
        Type de course: ${item.type_courses}
        prix: ${item.prix}
        Date de la course: ${formatDateTime(item.dateDeLivraison)}
      `);
    };
  ...
  useEffect(() => {
    ...
    onAuthStateChanged(auth, async (authUser) => {
      if (authUser) {
        ...
        const userDocRef = doc(db, "zems", userId);
